...
spec:
  opentelemetry:
    tracing: "on" <1>
    metrics: "on" <2>
    collector: TODO: address of the external collector <3>
----

<1> use `tracing: "on"` to enable propagation of traces. This will expect traces using the Jaegger protocol over gRPC sent to the port *14250*. Make sure you also configure the application to xref:telemetry:prometheus-backend.adoc[report traces to Jaegger].
...
<3> use `collector: TODO` to set the location of the external collector where the side-car will send the data to.
