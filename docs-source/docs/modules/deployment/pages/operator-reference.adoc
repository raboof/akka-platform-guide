...
The Akka Operator supports several modes of telemetry reporting. First, there's the classic architecture where the application directly pushes traces or exposes a prometheus port. Second, is the architecture based on the OpenTelemetry side-car.

[#reporting_metrics_to_prometheus]
=== Reporting Metrics to Prometheus (Classic)

In the classic architecture your application exports directly with a telemetry (metrics or tracing) backend using a trasnport that's custom for each backend.
...
=== OpenTelemetry side-car (defaults)

Using the OpenTelemetry side-car, the Pod where your application container is deployed will emit metrics and traces using the standard OTLP format. Then, an OpenTelemetry Collector can receive that OTLP data and adapt it to you preferred backends.

The OpenTelemetry side-car:

. MAY process Jaegger traces and forward them as OTLP traces
. MAY scrape the application containers to extract prometheus metrics and forward those them as OTLP metrics
. WILL send all the collected data to an external OTLP Collector

The OTLP Collector is a stand-alone process you will have to install separately. This standalone process may store the OTLP data or propagate it to specific backends such as Grafana, AWS CloudWatch, AWS X-Ray, GCP Cloud Trace, Google Cloud Monitoring,...

NOTE: You can read more about the https://opentelemetry.io/docs/concepts/components/[OpenTelemetry components  {tab-icon}, window="tab"] and the https://opentelemetry.io/registry/?s=collector[OpenTelemetry collectors  {tab-icon}, window="tab"] on the OpenTelemetry docs.

Configure the application CR to add the OpenTelemetry side-car to the application Pods.

The Akka Operator configures the OpenTelemetry side-car with a few defaults and only needs you to enable or disable a couple of fetaures and indicate the location of the collector:
...
spec:
  opentelemetry:
    tracing: "on" <1>
    metrics: "on" <2>
    collectorUrl: TODO: address of the external collector <3>
----

<1> use `tracing: "on"` to enable propagation of traces. This will expect traces using the Jaegger protocol using the https://www.jaegertracing.io/docs/1.22/client-features/[Thrift Compact {tab-icon}, window="tab"] format over UDP sent to the port *6831*. Make sure you also configure the application to xref:telemetry:jaegger-backend.adoc[report traces to Jaegger].
<2> use `metrics: "on"` to enable propagation of metrics. This will scrape Prometheus data on the port *9001* of the application container. Make sure you also configure the application to xref:telemetry:prometheus-backend.adoc[report metrics to Prometheus].
<3> use `collectorUrl: TODO` to set the location of the external collector where the side-car will send the data to.

By default, both `metrics` and `tracing` are `off`. If you don't enable at least one of the two the side-car will not be added.

[#opentelemetry_sidecar_custom]
=== OpenTelemetry side-car (custom)

For more advanced tuning of the OpenTelemetry side-car you can provide your own configuration via a secret:
...
spec:
  opentelemetry:
    configSecret: opentelemetry-sidecar-config <1>
----

<1> A secret containing the YAML configuration for the OpenTelemetry agent.

Providing your configuration will overwrite the one provided by the operator so the properties `tracing`, `metrics` and `collector` of the <<opentelemetry_sidecar>> will be ignored.

Refer to the https://opentelemetry.io/docs/collector/configuration/[documentation for the OpenTelemetry agent] to set up the OpenTelemetry side-car to your needs.
