...
=== OpenTelemetry side-car

Using the OpenTelemetry side-car, the Pod where your application container is deployed will emit OTLP metrics and traces. Then, an OpenTelemetry Collector can receive that OTLP data and share it to you preferred backends.
...
<1> use `tracing: "on"` to enable propagation of traces. This will expect traces using the Jaegger protocol over gRPC sent to the port *14250*.
<2> use `metrics: "on"` to enable propagation of metrics. This will scrape Prometheus data on the port xref:telemetry:prometheus-backend.adoc[*9001*] of the application container.
<3> use `collector: TODO` to set the location of the external collector where the side-car will send the data to.

Once you've enabled the side-car to forward metrics and traces, you must make sure the application produces that data. First, set up your application to use xref:telemetry:index.adoc[Lightbend Telemetry]. Then, xref:telemetry:prometheus-backend.adoc[configure the application to report metrics to Prometheus]. Finally, xref:telemetry:prometheus-backend.adoc[configure the application to report traces to Jaegger].

For more advanced tuning of the OpenTelemetry side-car you can provide your own configuration:
...
spec:
  opentelemetry:
    configSecret: opentelemetry-sidecar-config
----

Providing your configuration will overwrite the once provided by the operator so the properties `tracing`, `metrics` and `collector` will be ignored.
