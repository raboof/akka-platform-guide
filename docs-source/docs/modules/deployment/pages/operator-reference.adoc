...
Using the OpenTelemetry side-car the Pod where your application contrainer is deployed will emit OTLP metrics and traces. Then, an OTel Collector can receive that OTLP data and share it to you preferred backends.

NOTE: You can read more about the components on the https://opentelemetry.io/docs/concepts/components/[OpenTelemetry docs  {tab-icon}, window="tab"]

Configure the application CR to add the OpenTelemetry side-car to the Pods:
...
spec:
  opentelemetry:
    collector: TODO-address
----

Once the Pod is ready to forward metrics and traces you must make sure the application produces that data. First, setup your application to use xref:telemetry:index.adoc[Lightbend Telemetry]. Then, xref:telemetry:prometheus-backend.adoc[configure the application to report metrics to Prometheus]. Finally,, xref:telemetry:prometheus-backend.adoc[configure the application to report traces to Jaegger].
